...
| **Límites automáticos de asignación de tonos** | Controla cómo deben escribirse los valores de curvatura en la textura. Si se habilita, el rango de valor se normalizará entre 0 y 1 en función del valor mínimo y máximo encontrado durante el proceso de hacer un bake. Si está desactivado, los valores mínimo y máximo se definen manualmente.  **Nota:** Al hacer un bake UDIM/Mosaicos de UV, este parámetro debe deshabilitarse para que la asignación de tonos sea uniforme y no específica por mosaico; de lo contrario, esto podría crear uniones entre cada textura. Para encontrar manualmente los valores mínimos y máximos correctos, haga un bake primero con esta configuración habilitada y, a continuación, examine la consola/registro para ver qué valores generó el baker. |
| **Mínimo de asignación de tonos** | Si **Límites automáticos de asignación de tonos** está deshabilitado, define el valor mínimo para escalar el resultado de curvatura de modo que se ajuste a la textura. |
| **Máximo de asignación de tonos** | Si **Límites automáticos de asignación de tonos** está deshabilitado, define el valor máximo para escalar el resultado de curvatura y ajustarlo a la textura. |
